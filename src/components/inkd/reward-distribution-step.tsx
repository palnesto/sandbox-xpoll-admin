...
const DEFAULT_REWARD_TYPE: RewardItem["rewardType"] = "max";
      ...
      <div className="flex w-full items-center justify-between rounded-3xl border border-[#E6E7EB] bg-white px-5 py-4">
        ...
        >
          {rewardFields.map((field, index) => {
            ...
            const rowError = (errors.rewards?.[index] as {
              amount?: { message?: string };
              rewardAmountCap?: { message?: string };
            }) ?? {};
            const amountError = rowError.amount?.message;
            const capError = rowError.rewardAmountCap?.message;
                  ...
                  <div className="px-4 pb-5 pt-4">
                    ...
                    <div className="space-y-5">
                      <div className="space-y-1.5">
                        ...
                        />
                        <p className={cn("text-xs", capError ? "text-red-600" : "text-[#7A7A7A]")}>
                          {capError ?? "Must be greater than 0"}
                        </p>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        />
                        <p className={cn("text-xs", amountError ? "text-red-600" : "text-[#7A7A7A]")}>
                          {amountError ?? "Must be greater than 0"}
                        </p>
                        <Controller
                          control={control}
                          ...
                          render={({ field }) => (
                            <div className="flex items-center gap-8">
                              {/*
                                Keep max as default only when value is unset;
                                preserve explicit user choice ("min" or "max").
                              */}
                              {(() => {
                                const selectedRewardType =
                                  (field.value as RewardItem["rewardType"] | undefined) ??
                                  DEFAULT_REWARD_TYPE;
                                return (
                                  <>
                              <button
                      ...
                      {assetId && (
                        <div className="mt-4 border border-[#E6E7EB] p-4 rounded-3xl">
                          <RewardsAccordion
                            ...
                            rewardType={
                              ((watch(`rewards.${index}.rewardType`) as RewardItem["rewardType"] | undefined) ??
                                DEFAULT_REWARD_TYPE)
                            }
                    ...
                    {errors.rewards?.[index] && !capError && !amountError && (
