...
function baseToParentStr(assetId: AssetType, baseVal: string): string {
  if (!baseVal) return "";
  const r = amount({
    op: "toParent",
    assetId,
...
function parentToBaseStr(assetId: AssetType, parentStr: string): string {
  ...
  return r.ok ? String(r.value ?? "0") : "0";
}
...
function mapApiToFormValues(
  ...
  return {
    ...
    rewards: (d.rewards?.length
      ...
      assetId: r.assetId,
      // API stores amounts in base units; form expects parent (decimal) strings.
      amount: baseToParentStr(r.assetId as AssetType, r.amount ?? ""),
      rewardAmountCap: baseToParentStr(
  ...
  const onSubmit = async (values: InkdAgentCreateFormValues) => {
    ...
    const payload = {
      ...
      rewards: values.rewards.map((r) => ({
        assetId: r.assetId,
        // Convert parent (decimal) strings -> base units for API payload.
        amount: parentToBaseStr(r.assetId as AssetType, r.amount),
        rewardAmountCap: parentToBaseStr(
          r.assetId as AssetType,
          r.rewardAmountCap,
        ),
