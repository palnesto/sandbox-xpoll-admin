...
import { ArrowLeft, FileText, FileX, Link as LinkIcon, Trash2, X } from "lucide-react";
...
export default function InkdBlogEditPage() {
  ...
  const { data: blogResp, isLoading: blogLoading, isError: blogError } = useApiQuery(blogRoute, {
  ...
  if (!blogLoading && (blogError || !blog || !blog._id)) {
    return (
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="rounded-full bg-[#ececec] text-[#2a2a2a] border-b-2 border-b-white px-2 py-2 hover:bg-black/20 border-l-2 border-white/80"
              aria-label="Back"
            >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex h-[34px] w-[34px] items-center justify-center rounded-full border border-[#E6E7EB] bg-white text-red-500 hover:bg-red-50 hover:border-red-200 hover:text-red-600 transition-colors disabled:opacity-50"
