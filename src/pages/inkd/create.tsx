...
export default function CreateInkdInternalAgent() {
  ...
  useEffect(() => {
    if (activeStepId !== "settings") return;
    // Validate settings fields on entry so Next stays disabled until valid.
    void form.trigger([
      "maxBlogDescriptionLength",
      "maxLinkedTrial",
      "maxLinkedPoll",
  ...
  useEffect(() => {
    if (activeStepId !== "brand") return;
    // Validate brand field on entry so Next stays disabled until valid.
    void form.trigger(["brandLanguage"]);
  }, [activeStepId, form]);

  useEffect(() => {
    if (activeStepId !== "rewards") return;
    // Validate rewards on entry so Launch button stays disabled until all rows are valid.
    void form.trigger(["rewards"]);
  }, [activeStepId, form]);
  ...
  const hasInvalidRewardValues = rewards.some((r) => {
    const assetId = r?.assetId as AssetType | undefined;
    if (!assetId) return true;
    const amtR = amount({ op: "toBase", assetId, value: r.amount || "0", output: "bigint" });
    const capR = amount({ op: "toBase", assetId, value: r.rewardAmountCap || "0", output: "bigint" });
    if (!amtR.ok || !capR.ok) return true;
    return amtR.value <= 0n || capR.value <= 0n || capR.value < amtR.value;
  });

  const canGoNext =
    !isSubmitting &&
    !hasStepErrors(activeStepId, errors) &&
    (activeStepId !== "rewards" ? true : !hasInvalidRewardValues) &&
  ...
  const onSubmit = async (values: InkdAgentCreateFormValues) => {
    ...
    setCityOpts([]);
    setIndustryOpts([]);
    setSearchParams({ tab: STEP_ID_TO_TAB_PARAM.foundational }, { replace: true });

    const created = data?.data;
    if (created?.internalAgentId) {
      appToast.success("Signal AI launched successfully");
      navigate("/inkd");
    }
  };
