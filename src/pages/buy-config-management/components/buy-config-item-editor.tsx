...
export function BuyConfigItemEditor({
  ...
  const supportsSubscriptionCryptoConfig =
    item.supportsSubscriptionCryptoConfig === true;
  ...
  const schema = useMemo(
    () =>
      createItemSchema(
        limits,
        isAsset,
        supportsSubscriptionCryptoConfig,
      ),
    [isAsset, limits, supportsSubscriptionCryptoConfig],
  );
  ...
  const subscriptionUsdcMajor =
    useWatch({ control: form.control, name: "subscriptionUsdcMajor" }) ?? "";
  const subscriptionCadence = item.buyConfig.subscription?.cadence ?? null;
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} noValidate className="space-y-4">
      ...
      <div
        className={[
          ...
          supportsSubscriptionCryptoConfig ? "lg:grid-cols-4" : "lg:grid-cols-3",
        ].join(" ")}
      >
        ...
        {supportsSubscriptionCryptoConfig ? (
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-3 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm">Subscription Enable</span>
              <Controller
                control={form.control}
            ...
            <p className="text-xs text-zinc-400">
              Main on/off for recurring crypto billing on this item.
            </p>

            <div className="flex items-center justify-between">
              <span className="text-sm">Subscription Crypto Rail</span>
              <Controller
                control={form.control}
                name="subscriptionCryptoEnable"
                render={({ field }) => (
                  <Switch
                    checked={field.value === true}
                    onCheckedChange={(checked) => field.onChange(checked)}
            ...
            <p className="text-xs text-zinc-400">
              Turns on recurring USDC support for this item.
            </p>

            <div className="flex items-center justify-between">
              <span className="text-sm">Subscription USDC Price Enable</span>
              <Controller
                control={form.control}
                name="subscriptionUsdcEnable"
                render={({ field }) => (
                  <Switch
                    checked={field.value === true}
                    onCheckedChange={(checked) => field.onChange(checked)}
            ...
            <p className="text-xs text-zinc-400">
              Allows recurring checkout in USDC when subscription crypto is enabled.
            </p>

            <TextField<BuyConfigFormValues>
              form={form}
              schema={schema}
              name="subscriptionUsdcMajor"
              label="Subscription USDC Price"
              placeholder={`<= ${limits.maxUsdcMajor}`}
              helperText={`Enter the recurring price in USDC. Example: 1 means one USDC. Decimals are allowed (like 149.5). Max allowed is ${limits.maxUsdcMajor}.`}
              showError
            />

            {toMinorPreview(subscriptionUsdcMajor, limits.usdcMinorScale) ? (
              <p className="text-[11px] text-zinc-500">
                Stored automatically in system units as{" "}
                {toMinorPreview(subscriptionUsdcMajor, limits.usdcMinorScale)}.
              </p>
            ...
            {subscriptionCadence ? (
              <p className="text-[11px] text-zinc-500">
                Billing cadence is read-only here: every{" "}
                {subscriptionCadence.intervalCount}{" "}
                {subscriptionCadence.intervalUnit}
                {subscriptionCadence.intervalCount === 1 ? "" : "s"}.
              </p>
            ) : (
              <p className="text-[11px] text-zinc-500">
                Billing cadence is not configured on this item.
              </p>
            )}
          </div>
        ) : null}
