...
export function toFormDefaults(
  ...
  const subscription = item.buyConfig.subscription;
  ...
  return {
    ...
    subscriptionEnable: subscription?.enable === true,
    subscriptionCryptoEnable: subscription?.crypto.enable === true,
    subscriptionUsdcEnable: subscription?.crypto.usdc.enable === true,
    subscriptionUsdcMajor: toMajorInput(
      subscription?.crypto.usdc.rateInMinor ?? 0,
      limits.usdcMinorScale,
    ),
...
export function createItemSchema(
  limits: Limits,
  isAsset: boolean,
  supportsSubscriptionCryptoConfig: boolean,
) {
  return z.object({
    enable: z.boolean(),
    ...
    subscriptionEnable: z.boolean(),
    subscriptionCryptoEnable: z.boolean(),
    subscriptionUsdcEnable: z.boolean(),
    subscriptionUsdcMajor: supportsSubscriptionCryptoConfig
      ? createRateSchema({
...
export function toUpdateBody(
  ...
  const subscriptionUsdcMinorParsed = item.supportsSubscriptionCryptoConfig
    ? parseMajorToMinor(values.subscriptionUsdcMajor, limits.usdcMinorScale)
    : null;

  if (
    !usdMinorParsed.ok ||
    !usdcMinorParsed.ok ||
    (item.supportsSubscriptionCryptoConfig && (!subscriptionUsdcMinorParsed || !subscriptionUsdcMinorParsed.ok))
  ) {
  ...
  if (item.supportsSubscriptionCryptoConfig && subscriptionUsdcMinorParsed?.ok) {
    body.subscription = {
      enable: values.subscriptionEnable,
      crypto: {
        enable: values.subscriptionCryptoEnable,
        usdc: {
          enable: values.subscriptionUsdcEnable,
          rateInMinor: subscriptionUsdcMinorParsed.value,
        },
      },
    };
  }
