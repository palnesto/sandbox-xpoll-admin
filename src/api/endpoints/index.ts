export const endpoints = {
  ...
  entities: {
    ...
    queues: {
      bullMqJobRuns: (queryParams?: Record<string, string | number | boolean | undefined | null>) => {
        const baseUrl = "/internal/queues/bullmq-job-runs";
    ...
    industry: {
      ...
      commonAdvancedListing: "/common/industry/advanced-listing",
    ...
    inkd:{
      topIndustries: {
        list: "/common/inkd/top-industries",
        add: "/internal/inkd/top-industries",
        remove: (industryId: string) => `/internal/inkd/top-industries/${industryId}`,
      },
      chat: {
        send: "/internal/inkd-internal-agents/chat",
        list: (queryParams?: Record<string, string | number | boolean | undefined | null>) => {
          const baseUrl = "/internal/inkd-internal-agents/chat";
          const searchParams = new URLSearchParams();
          Object.entries(queryParams ?? {}).forEach(([key, value]) => {
            if (value === undefined || value === null || value === "") return;
            searchParams.set(key, String(value));
        ...
        getById: (chatId: string) => `/internal/inkd-internal-agents/chat/${chatId}`,
        updateMetadata: (chatId: string) =>
          `/internal/inkd-internal-agents/chat/${chatId}/metadata`,
        history: (chatId: string, queryParams?: Record<string, string | number | boolean | undefined | null>) => {
          const baseUrl = `/internal/inkd-internal-agents/chat/${chatId}/history`;
          const searchParams = new URLSearchParams();
          Object.entries(queryParams ?? {}).forEach(([key, value]) => {
            if (value === undefined || value === null || value === "") return;
            searchParams.set(key, String(value));
        ...
        activeJob: (chatId: string) => `/internal/inkd-internal-agents/chat/${chatId}/active-job`,
        jobEvents: (chatId: string, jobId: string) =>
          `/internal/inkd-internal-agents/chat/${chatId}/jobs/${jobId}/events`,
        cancelJob: (chatId: string, jobId: string) =>
          `/internal/inkd-internal-agents/chat/${chatId}/jobs/${jobId}/cancel`,
      },
      internalAgent:{
      ...
      blogs:{
        ...
        createWithTrials: "/internal/inkd-internal-agents/blogs/with-trials",
        ...
        analytics: (blogId: string, queryParams?: Record<string, string>) => {
          const baseUrl = `/internal/inkd-internal-agents/blogs/${blogId}/analytics`;
          const searchParams = new URLSearchParams(queryParams ?? {}).toString();
